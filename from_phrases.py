...
def song_plan_from_phrases(phrases: list[str], genre: str, output_dir: pathlib.Path, detailed_composition_plan: bool = False) -> CompositionPlan:
...
"""
        ...
        print(f"Extracted {len(timestamped_words)} words with timestamps")
...
if __name__ == "__main__":
    ...
    genre = "a raw, emotionally charged track that fuses alternative R&B, gritty soul, indie rock, and folk. The song should still feel like a live, one-take, emotionally spontaneous performance."
    #print(plan)
    resources_path = pathlib.Path("../resources")
    plan = song_plan_from_video(video_url, genre, output_dir=resources_path / "muromati_v3")
